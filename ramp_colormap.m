function colors = ramp_colormap(limits, step_value)
% RAMP_COLORMAP colormap with clear discrete colors and ramp in brightness
% within each color
%
% colors = RAMP_COLORMAP(limits, step_value)

if nargin == 0 || numel(limits) == 0
    limits = caxis;
end
if nargin < 2
    step_value = 1;
...
value_arr = linspace(min(limits), max(limits), n_pts);

for idx = 1:n_pts
    value = value_arr(idx);
    hue_idx = floor(abs(value)/step_value) + 1;
    lum_value = rem(-abs(value), step_value)/step_value + 1;
    lum = 0.5 + (lum_value-0.5)*(1-lum_buffer);
    colors(idx, 1) = hue_idx;
    colors(idx, 2) = 0.9;
    colors(idx, 3) = lum;
end
